...
function _build() {
  _addFace(0, 1, 4, 3, vec3(0, 0, 1));
  _addFace(1, 4, 5, 2, vec3(1, 0, 0));
  _addFace(0, 3, 5, 2, vec3(0, 1, 0));
  _addSide(3, 4, 5, normalize(vec3(0, 1, 2)));
  _addSide(0, 1, 2, normalize(vec3(0, 1, -2)));
}
...
function draw(gl, program, primitive) {
  ...
  gl.bindBuffer(
    ...
    primitive == gl.LINES ? edges_buffer : faces_buffer
  );
  gl.drawElements(
    primitive,
    primitive == gl.LINES ? edges.length : faces.length,
    gl.UNSIGNED_BYTE,
    0
  );
}
